...
then
    ...
    scp -q -o "StrictHostKeyChecking no" -o "UserKnownHostsFile /dev/null" -r $lynis_directory/lynis/ "$LOGIN_AS_USER"@$VM_IP:/tmp > /tmp/scp.log 2>&1
    if [ "$?" -eq "0" ];
    then
        if ! ssh -q -o "StrictHostKeyChecking no" -o "UserKnownHostsFile /dev/null" "$LOGIN_AS_USER"@$VM_IP 'bash -s' < lynis-client.sh > $FOLDER_PATH/lynis_test.txt; then
